...
const ROOM_UPDATE_API = `${API_URL}/room_update`;
const ROOM_LOG_CLEAN_API = `${API_URL}/attendant`;
const ROOM_NOTES_API = `${API_URL}/room_notes`;
const IMAGE_UPLOAD_API = 'http://api.roomchecking.com:3054/image-upload';
const LOST_ITEM_API = `${API_URL}/lost_found/founds`;
const INVENTORY_API = `${API_URL}/hotel_inventory`;
const TASK_API = `${API_URL}/tasks`;
const VIRTUAL_ASSETS_API = `${API_URL}/virtual_assets`;

// Update Room
export function * updateRoom({ roomId, status }) {
...
export function * logClean({ roomId }) {
...
export function * logOther({ roomId, status }) {
  const { auth: { hotelId, token, user }, updates: { rooms: roomUpdates }, rooms: { hotelRooms }} = yield select();
  const room = find(hotelRooms, { _id: roomId });
...
export function * updateRoomFlow({ roomId, status }) {
  try {
...
export function * createRoomNoteFlow({ roomId, note }) {
  try {
...
export function * watchCreateRoomNote(state) {
  yield * takeEvery(UpdatesTypes.ROOM_NOTE_ADD, createRoomNoteFlow);
}

export function * removeRoomNote({ roomId, noteId }) {
  const { auth: { hotelId, token, user, userId } } = yield select();
...
export function * removeRoomNoteFlow({ roomId, noteId }) {
  try {
...
export function * watchRemoveRoomNote() {
  yield * takeEvery(UpdatesTypes.ROOM_NOTE_REMOVE, removeRoomNoteFlow);
}

// Image Upload
export function uploadPhoto({ path }) {
...
export function * uploadPhotoFlow({ path, id }) {
  try {
...
export function * submitLostItem({ desc, roomId, photoId }) {
...
export function * addLostItemFlow({ desc, roomId, photoId}) {
...
export function * watchAddLostItem() {
  yield * takeEvery(UpdatesTypes.LOST_ITEM_ADD, addLostItemFlow);
}

export function * submitInventoryWithdrawl({ asset, change, roomId }) {
  const { auth: { hotelId, token, user, userId } } = yield select();
...
export function * submitInventoryWithdrawlFlow({ roomId }) {
...
export function * watchSubmitInventoryWithdrawal() {
  yield * takeEvery(UpdatesTypes.INVENTORY_FLUSH, submitInventoryWithdrawlFlow);
}

// Hotel Inventory Restock
export function * submitInventoryRestock({ assetRoomId, id }) {
  const { auth: { hotelId, token, user, userId } } = yield select();
...
export function * submitInventoryRestockFlow({ assetRoomId, id }) {
...
export function * watchSubmitInventoryRestock() {
...
export function * createVirtualAsset({ name, image }) {
  const { auth: { hotelId, token, userId } } = yield select();
...
export function * watchCreateTaskFlow() {
...
export function * updateTaskFlow({ uuid, status }) {
  try {
...
export function * watchUpdateTaskFlow(state) {
