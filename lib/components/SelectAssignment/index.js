...
class ModalAssignment extends Component {
  ...
  render() {
    const { assignmentOptions, selectedAssignments, updateAssignments, removeAssignment, isShowHeader } = this.props
    const selected = compact(selectedAssignments.map(a => assignmentOptions.find(ao => ao.value === a)))

    return (
      <View>
        { isShowHeader && <SectionHeader value={I18n.t('base.ubiquitous.assignments')} /> }
        <Opener
