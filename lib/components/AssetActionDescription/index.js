...
class AssetActionDescription extends Component {
  ...
  _renderLocation() {
    ...
    if (room) {
      return (
        <View>
          <SectionHeading style={{ paddingLeft: 15 }}>Location</SectionHeading>
  ...
  _renderSelectAsset() {
    ...
    const displayText = selectedAsset ? selectedAsset.name : createdAsset ? createdAsset : `Select the asset`;

    return (
      <View>
        <SectionHeading style={{ paddingLeft: 15 }}>Asset</SectionHeading>
  ...
  _renderSelectAction() {
    ...
    let rendered;
    if (createdAsset) {
      rendered = <Text style={styles.actionError}>No actions for created asset</Text>;
    } else if (!selectedAsset) {
      rendered = <Text style={styles.actionError}>You must first select an asset</Text>;
    } else if (!buttons || !buttons.length) {
      rendered = <Text style={styles.actionError}>There are no actions for your asset</Text>;
    } else {
      rendered = buttons;
    }
    ...
    return (
      <View>
        <SectionHeading style={{ paddingLeft: 15 }}>Action</SectionHeading>
  ...
  _renderDesc() {
    ...
    return (
      <View>
        <SectionHeading style={{paddingLeft: 15}}>Description</SectionHeading>
        <TextInput
          ...
          multiline={true}
          maxLength={200}
          placeholder={"Ex. It wont turn on."} />
      </View>
    )
  }
  ...
  _renderPriority() {
    ...
    return (
      <View>
        <SectionHeading style={{paddingLeft: 15}}>Priority</SectionHeading>
        <View style={styles.priorityContainer}>
          <TouchableOpacity style={[styles.priorityBtn, isPriority ? { backgroundColor: '#3CC86B' } : null]} onPress={() => handleUpdatePriority(true)}>
            <Text style={styles.bigBtnText}>Yes</Text>
          </TouchableOpacity>
      ...
      <View style={[styles.container, style]}>
        ...
        { handleSubmit ?
          <Button style={{ height: 50, backgroundColor: '#3CC86B', borderRadius: 5 }} onPress={handleSubmit}>
            <Text style={styles.bigBtnText}>{ `Submit Maintenance`.toUpperCase() }</Text>
          </Button>
          : null
        }
