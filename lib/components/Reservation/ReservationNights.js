...
const ReservationNights = ({step, total}) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={[styles.text, { marginRight: 2, marginLeft: 2 }]}>{ I18n.t('attendant.components.reservation-nights.of') }</Text>
      <View style={styles.totalIndicator}>
        <Text style={[styles.text, styles.whiteText]}>{ total }</Text>
      </View>
      <Text style={[styles.text, { marginLeft: 2}]}>{ I18n.t('attendant.components.reservation-nights.nights') }</Text>
    </View>
  )
}
