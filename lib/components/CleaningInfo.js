...
const CleaningInfo = ({ room, activeId = null, config = {}, isTurndown = false, isRunner = false, style, inline = false }) => {
  if (!room) {
    return null;
  }
  ...
  return (
    <View style={[styles.container, style]}>
      <View style={{ marginTop: 15 }}>
        <Text style={[styles.infoHeader, inline && { marginLeft: 0, marginBottom: 10 }]}>{ I18n.t('attendant.clean.cleaninginfo.cleaning-info').toUpperCase() }</Text>
        <View style={[styles.infoContainer, inline && { marginLeft: 0, marginRight: 0 }]}>
