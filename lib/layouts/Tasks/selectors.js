...
import { tasksSelector, allHotelRoomsIndex } from '../../selectors/rooms';
...
export const assignedTasks = createSelector(
  [assignedTasksBase],
  (assigned) => {
    const mapped = assigned.map((task) => ({...task, category: Filtering.isPriority(task) ? 'priority' : task.room.name}))
...
export const sentTasks = createSelector(
  [sentTasksBase],
  (sent) => {
    const mapped = sent.map((task) => ({...task, category: Filtering.isClosed(task) ? 'closed' : 'open'}))
