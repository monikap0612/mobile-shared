...
import { computedDefaultPopupNotifications, computedAttendantPopupNotifications, computedRunnerPopupNotifications } from './selectors';
...
class PopupLayout extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    const { viewedTasks: nextViewedTasks } = nextState;
  ...
  _renderGlitches() {
    ...
    return (
      <View>
        <View style={styles.header}>
          <View style={styles.headerSide}></View>
          <Text style={styles.headerText}>NEW GLITCH</Text>
          <View style={styles.headerSide}></View>
        </View>
        <View style={{ flex: 1 }}>
          <ListView
  ...
  render() {
    const { tasks, notifications, glitches } = this.props;
    const { viewedTasks } = this.state;
    ...
    return (
      <Modal
        ...
        { !notifications.length && !tasks.length && glitches.length ?
