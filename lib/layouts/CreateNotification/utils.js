...
export const buildNotifications = (values, users) => {
  const { user, locations, message, photo } = values;
  const selectedValue = user.value;
  const isAnySelectedGroups = user.isGroup

  let notifications = [];

  if (!locations.length && selectedValue !== 'planned') {
    return []
  }
  ...
  if (selectedValue === 'planned') {
    ...
    selectedValue.users.forEach(userId => {
    ...
    locations.map(room => {
      notifications.push({ message, photoUrl: photo.path, photoPath: photo.id, room, user: selectedValue });
    });
  }
