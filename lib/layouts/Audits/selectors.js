...
export const allAudits = createSelector(
  ...
  (audits, auditSources) => {
    ...
    const paused = valid.filter(a => a.status === 'paused')
    const open = valid.filter(a => a.status === 'open')
    const completed = valid.filter(a => a.status === 'completed')
