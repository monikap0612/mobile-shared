...
export const drawerDisabledSelector = createSelector(
  ...
  (user, config) => {
    ...
    const {
      ...
      isDisableAttendantAudits,
      isDisableAttendantPlannings,
    ...
    if (isAttendant) {
      ...
      if (isDisableAttendantAudits) { disabled.push('Audits') }
      if (isDisableAttendantPlannings) { disabled.push('Plannings') }
