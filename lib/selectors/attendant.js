...
export const getAttendantPopupTasks = (hotelTasks, indexedRooms, userId) => {
  ...
  return hotelTasks.filter(task => {
    ...
    if (get(task, 'assigned.isPlannedAttendant') && has(indexedRooms, roomId)) {
      return true;
    }
...
export const computedAttendantPopupTasks = createSelector(
  [tasksSelector, computedIndexAttendantRooms, userIdSelector],
