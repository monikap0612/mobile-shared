...
const getComputedAssets = (hotelAssets, hotelVirtualAssets, hotelCustomActions) => {
  const sortedAssets = sortBy(concat(hotelAssets || [], hotelVirtualAssets || []), 'name');
...
export const computedAssets = createSelector(
  [assetsSelector, virtualAssetsSelector, customActionsSelector],
  getComputedAssets
);
...
export const getAssetsByRoomId = (id) => createSelector(
  [computedAssetsIndex, computedGroupedAssetRooms, updatesInventorySelector], 

  (hotelAssetsIndex, hotelAssetRoomsGrouped, inventoryUpdates) => {
