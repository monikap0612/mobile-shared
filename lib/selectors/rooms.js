...
export const getOpenTasks = (hotelTasks, userId) => {
  ...
  return filter(hotelTasks, task => !get(task, 'is_completed') && !get(task, 'is_cancelled') && get(task, 'type') !== 'notification' && !get(task, 'is_rejected') && !includes(get(task, 'assigned.rejected_ids', []), userId));
}
...
export const getPopupTasks = (hotelTasks, userId) => {
  ...
  return filter(hotelTasks, task => today === get(task, 'due_date') && !get(task, 'is_claimed') && !get(task, 'is_rejected') && !includes(get(task, 'assigned.rejected_ids', []), userId));
}
