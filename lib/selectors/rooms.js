...
import { get, extend, values } from 'lodash/object';
...
export const selectPlanningsUpdates = state => state.planningUpdates;
...
export const currentUserPlanningsUpdatesSelector = createSelector(
  [planningsUpdatesSelector, userIdSelector],
  (updates, userId) => {
    return updates.filter(val => val.planning_user_id === userId)
  }
)
