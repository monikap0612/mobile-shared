...
const ACTION_HANDLERS = {
  ...
  [UpdatesTypes.SAVE_OUTGOING_HASH]: (state, { hash }) => {
    let currentHashes = state.outgoingHashes;
    if (!isArray(currentHashes)) {
      currentHashes = [];
    }
    updatedHashes = [hash,].concat(currentHashes)
    
    return {
      ...state,
      outgoingHashes: updatedHashes.slice(0, OUTGOING_HASHES_LIMIT)
    }
  }
}
