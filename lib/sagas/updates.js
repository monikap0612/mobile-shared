...
export default function({ apiUrl }) {
  ...
  // Update Room
  const updateRoom = offlineable('updateRoom', function * updateRoom ({ roomId, status, tapTs }) {
  ...
  function * updateRoomFlow({ roomId, status, tapTs }) {
    const { updates: { outgoingHashes: oldHashes }} = yield select();
    const hash = ruHash(roomId, 'attendantStatus', status);
    console.log(hash, oldHashes);
    if (hash.includes('noValue')) {
      if (oldHashes.length && oldHashes[0].includes('noValue')) {
        console.log('ditching since they match');
        return true;
      }
    } else if (hash.includes('pause')) {
      if (oldHashes.length && oldHashes[0].includes('pause')) {
        console.log('ditching since they match');
        return true;
      }
    } else if (oldHashes.length && oldHashes.includes(hash)) {
      console.log('ditching since they match');
      return true;
    }
    ...
    try {
      ...
      const response = yield call(updateRoom, { roomId, status, tapTs });
      ...
      yield put(UpdatesActions.saveOutgoingHash(hash));      
      
      if (response && response.room) {
        yield put(RoomsActions.updateRoom(response.room))
      }
  ...
  // Reset Room
  const resetRoom = offlineable('resetRoom', function * resetRoom(roomId, tapTs = null) {
    const user = yield select(userSelector);
  ...
  // Room Model
  const updateRoomModel = function * updateRoomModel({ roomId, field, value, tapTs }) {
    const user = yield select(userSelector);
  ...
  const updateRoomModelFlow = offlineable('updateRoomModelFlow', function * updateRoomModelFlow({ roomId, field, value, tapTs }) {
    const { updates: { outgoingHashes: oldHashes }} = yield select();
    const hash = ruHash(roomId, field, value);
    console.log(hash, oldHashes);
    if (oldHashes && oldHashes.includes(hash)) {
      console.log('ditching since they match');
      return true;
    }

    yield put(RoomsActions.updateRoomOptimistic({ roomId, field, value }));
    const response = yield call(updateRoomModel, { roomId, field, value, tapTs });
  ...
  const createTaskBatchFlow = offlineable('createTaskBatchFlow', function * createTaskFlow({ data, tapTs }) {
    yield put(UpdatesActions.taskSending());
    const { auth: { hotelId, token, userId, hotel }, updates: { photos }, users: { users, hotelGroups } } = yield select();
    let image = get(data, 'asset.image', null);
    ...
    const response = yield call(createTaskBatch, task, tapTs);
  ...
  const updateTaskFlow = offlineable('updateTaskFlow', function * updateTaskFlow({ uuid, status, tapTs }) {
    ...
    const { task } = yield call(updateTask, { uuid, status, tapTs });
    return true;
  })
