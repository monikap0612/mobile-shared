...
export default function({ apiUrl }) {
  ...
  function * updateFoundItemFlow({ id, item }) {
    try {
  ...
  function * photoFoundItemFlow({ id, item, field, photoId }) {
    try {
  ...
  function * submitInventoryWithdrawlFlow({ roomId }) {
    ...
    try {
      ...
      // filteredAssets.forEach(asset => OutboundActions.inventoryWithdrawal(asset, -1 * get(roomChanges, asset._id, 1), roomId));
      // for (const asset of filteredAssets) {
      //   try {
      //     yield put(OutboundActions.inventoryWithdrawal(asset, -1 * get(roomChanges, asset._id, 1), roomId))
      //   } catch (error) {
      //     console.log(error);
      //   }
      // }
