...
export default function({ apiUrl }) {
  ...
  function * watchUpdateRoomPause(state) {
    yield takeEvery(UpdatesTypes.ROOM_CLEAN_PAUSE, updateRoomFlow);
  }

  function * watchUpdateRoomUnpause(state) {
    yield takeEvery(UpdatesTypes.ROOM_CLEAN_UNPAUSE, updateRoomFlow);
  }
  ...
  function * watchUpdateRoomInspect(state) {
    yield takeEvery(UpdatesTypes.ROOM_INSPECT, updateRoomFlow);
  }
  ...
  const createTaskBatchFlow = offlineable(function * createTaskFlow({ data }) {
    const { auth: { hotelId, token, userId, hotel }, updates: { photos }, users: { users, hotelGroups } } = yield select();
    let image = get(data, 'asset.image', null);
  ...
  function * watchCreateTaskBatchFlow() {
    yield takeEvery(UpdatesTypes.TASK_CREATE_BATCH, createTaskBatchFlow);
  }
  ...
  const convertTaskFlow = offlineable(function * convertTaskFlow({ task, update }) {
    yield call(convertTask, { task, update });
  });

  function * watchConvertTaskFlow(state) {
    yield takeEvery(UpdatesTypes.TASK_CONVERT, convertTaskFlow);
  }

  const createNotificationFlow = offlineable(function * createNotificationFlow({ user, message, room, photoUrl, photoId, photoPath }) {
