...
export default function({ apiUrl }) {
  ...
  // const updateTask = offlineable(function * updateTask({ uuid, status }) {
  //   const { auth: { hotelId, token, userId } } = yield select();
  //
  ...
  function * updateTaskFlow({ uuid, status }) {
    try {
      const response = yield call(updateTask, { uuid, status });
      return yield put(RoomsActions.taskUpdateSuccess({ task: response.task }));
    } catch(e) {
      console.log(e);
    } finally {
  ...
  const createNotificationFlow = offlineable(function * createNotificationFlow({ user, message, room }) {
