...
export default function({ apiUrl }) {
  ...
  // Reset Room
  const resetRoom = offlineable('resetRoom', function * resetRoom(roomId, tapTs) {
    const user = yield select(userSelector);
  ...
  // Hotel Tasks
  function * createTask(task, tapTs) {
  ...
  const createTaskFlow = offlineable('createTaskFlow', function * createTaskFlow({ data, tapTs }) {
    ...
    if (data.activeGlitch) {
      yield call(updateGlitch, { task_id: response.uuid, users_id: userId, glitchId: data.activeGlitch })
    }
  ...
  // Hotel Tasks
  function * createTaskBatch(task, tapTs) {
