...
class Socket {
  constructor(dispatch, options) {
    ...
    this._debounceRoomUpdate = debounce(() => {
      console.log('exec debounce rooms')
  ...
  _handleRoomUpdate(data) {
    ...
    if (isEnableWSBlock && data && get(data, 'session.user._id') === this.userId) {
      ...
      return this._debounceRoomUpdate();
    }
  ...
  _handlePusherConnected() {
    ...
    // this.dispatch(NetworkActions.statusOnline());
    // this.dispatch(OfflineActions.start());
  }

  _handlePusherDisconnected() {
    console.log('pusher disconnected');
    // this.dispatch(NetworkActions.statusOffline());
  }
}
