import { white, taskActionColors } from "rc-mobile-base/lib/styles";
...
const buildActivity = (type, children) => ({
  ...
  children:
    ...
    children.map((child, index) => ({
      ...child,
      icon: activityIcons[`${type}:${index}`],
      status: updateStatuses[`${type}:${index}`],
    })),
});
...
export const getTaskActivities = task => {
  ...
  if (task.type == "quick" && !task.meta.action) {
    const states = noAssetAction;
    const activities = states[activityState] || states["initial"];
    return activities;
  } else if (task.type == "quick" && task.meta.action) {
    const states = quickStates;
    const activities = states[activityState] || states["initial"];
    return activities;
  } else if (
    task.type == "quick" &&
    !task.meta.asset_id &&
    !task.meta.durable_asset_id &&
    !task.meta.virtual_asset_id &&
    task.meta.action
  ) {
    const states = noAssetAction;
    const activities = states[activityState] || states["initial"];
